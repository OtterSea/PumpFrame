...
public class IOEx  {
    ...
    static public string ReadAllTextFromStreamingAssets(string path)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            if (result == UnityEngine.Networking.UnityWebRequest.Result.ConnectionError || result == UnityEngine.Networking.UnityWebRequest.Result.ProtocolError)
            {
    ...
    static public byte[] ReadAllBytesFromStreamingAssets(string path)
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            ...
            if (result == UnityEngine.Networking.UnityWebRequest.Result.ConnectionError || result == UnityEngine.Networking.UnityWebRequest.Result.ProtocolError)
            {
                Debug.LogError(String.Format("ReadAllBytesFromStreamingAssets Failed {0} error{1}", path, file.error));
                bytes = null;
            }
            else
            {
